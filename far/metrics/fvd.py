...
class FrechetVideoDistance(nn.Module):
    ...
    def extract_features_in_batches(self, videos, device):
        ...
        for start in range(0, num_videos, batch_size):
            ...
            batch_videos = videos[start:end].to(device)
            feats_batch = self.detector(batch_videos, **self.detector_kwargs).cpu().numpy()
    ...
    def compute(self, videos_fake: torch.Tensor, videos_real: torch.Tensor,
        ...
        feats_fake = self.extract_features_in_batches(videos_fake, device)
        feats_real = self.extract_features_in_batches(videos_real, device)
